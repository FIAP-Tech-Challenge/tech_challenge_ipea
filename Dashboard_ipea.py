...
df_raw = df
df = df.rename(columns={"Preço - petróleo bruto - Brent (FOB) - US$ - Energy Information Administration (EIA) - EIA366_PBRENT366":"preco", "Data":"data"}) #Renomeando as colunas
...
tab1, tab2= st.tabs(['Introdução', 'Database'])
with tab1:  
    st.markdown("<h2 style='text-align: left; color: orange;'>O que nos foi proposto<h2>", unsafe_allow_html=True)
...
with tab2:
    st.markdown("<h2 style='text-align: left; color: orange;'>Base de Dados em Estudo<h2>", unsafe_allow_html=True)
